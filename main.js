...
const askForSecrets = async () => {
  ...
  if (
    ...
    let shouldUseTokens =
      (
        await promisedQuestion(
          "Do you want to use launcher account data? Y or N [N]: "
        )
      ).toLowerCase() === "y";
    ...
    if (!shouldUseTokens) {
      ...
      launcherPath =
        (await promisedQuestion(
          "Path to Minecraft Launcher data folder, leave blank to autodetect []: "
        )) || guessLauncherPath();
  ...
  console.log(localConf.discordBot);
  if (localConf.discordBot == true) {
...
function stop() {
  exec("termux-notification-remove 2b2w");
...
function startQueuing() {
  exec(
    'termux-notification --title 2bored2waitPhone --content "Queue: None" --priority default --ongoing --id 2b2w'
  );
  stopQueing();
  doing = "auth";
        ...
        if (!finishedQueue && config.minecraftserver.is2b2t) {
          ...
          if (
            ...
            if (config.get("userStatus")) {
              ...
              if (parseInt(positioninqueue) <= 30) {
                exec(
                  `termux-notification --title 2bored2waitPhone --content "Queue: ${positioninqueue}" --priority default --ongoing --id 2b2w`
                );
                exec(
                  'termux-notification --title 2bored2waitPhone --content "JOIN NOW THERE ARE LESS THAN 10 PERSON IN THE QUEUE" --priority high --id 2b2wWarning'
                );
              } else {
                exec(
                  `termux-notification --title 2bored2waitPhone --content "Queue: ${positioninqueue}" --priority default --ongoing --id 2b2w`
